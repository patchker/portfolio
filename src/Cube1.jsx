...
function Model({ modelPath, onLoaded }) {
    ...
    useEffect(() => {
        if (scene) {
            // Model załadowany, wywołaj onLoaded
            onLoaded();
        }
    }, [scene, onLoaded]); // Dodaj `onLoaded` do tablicy zależności, aby zapewnić, że będzie użyta aktualna wersja
    ...
    const { scale, position, rotation } = useSpring({
        ...
        to: async (next) => {
            // Sekwencja animacji...
            await next({ position: [0, 0, 0], rotation: [0, 0, 0], config: { duration: 1000 } });
            await next({
                position: [0, 0, 0],
...
function Lights() {
    ...
    useFrame((state) => {
        const x = state.mouse.x; // Zmniejszono zakres dla subtelniejszego efektu
        const y = state.mouse.y ;
        if (pointLightRef.current) {
            pointLightRef.current.position.x = x;
            pointLightRef.current.position.y = y;
            pointLightRef.current.position.z = 0; // Możesz dostosować tę wartość
        }
    });
    ...
    return (
        ...
        <Suspense fallback={null}>
            <Canvas antialias={true} style={{ width: '100%', height: '100%', backgroundColor: '#000000' }}>
                ...
                <Environment background={false} files="mapa6.hdr" path="/"/>
...
function handleMouseMove(event) {
    // Funkcja do obsługi ruchu myszy, jeśli potrzebujesz dodatkowej logiki
}
